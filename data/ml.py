...
#with open("data/bike_availability_model.pkl", "rb") as file:
...
"""def fetch_openweather_forecast(date):
    ...
    api_key = os.environ.get('OPENWEATHER_API_KEY')
    url = (f"api.openweathermap.org/data/2.5/forecast/daily?q=Dublin&cnt={16}&appid={api_key}"
        )
    response.raise_for_status()
    ...
    target_unix = int(date.replace(tzinfo=timezone.utc).timestamp())

    for day in data["list"]:
        # 'dt' is the UNIX timestamp for the forecast date
        if abs(day["dt"] - target_unix) < 12 * 3600:  # allow small time window
            return {
    ...
    return {"error": f"No forecast found for {date.strftime('%Y-%m-%d')}"}
